...
export NRPLOGDIR=/users/bp000231/bf_data/nestserver_benchmarks/tmp_benchmarks_results/2021-12-13_17-26-40-hpcbench_readspikes/results/9/nrplogs
...
sarus run \
	  ...
	  ln -s /users/bp000231/bf_data/nestserver_benchmarks/Experiments/RoboBrain_benchmark/1_nrpexperiment_robobrain_mouse/robobrain_mouse_with_joystick $NRPSRC/gzweb/http/client/assets/robobrain_mouse_with_joystick; \
          unset LD_PRELOAD; \
	  ...
	  sed -i 's|self.__remaining_time = timeout|self.__remaining = timeout|' $NRPSRC/ExDBackend/hbp_nrp_cleserver/hbp_nrp_cleserver/server/SimulationServer.py; \
          ...
          sed -i 's|NestClientControlAdapter()|NestClientControlAdapter(\"nid01261\", 5000)|' $NRPSRCSERV/server/ServerConfigurations.py;
          \
