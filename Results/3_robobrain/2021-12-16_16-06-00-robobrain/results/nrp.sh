...
export NRPLOGDIR=/users/bp000231/bf_data/nestserver_benchmarks/tmp_benchmarks_results/2021-12-16_16-06-00-robobrain/results/11/nrplogs
...
sarus run \
	  ...
	  sed -i 's|self.__remaining_time = timeout|self.__remaining = timeout|' $NRPSRC/ExDBackend/hbp_nrp_cleserver/hbp_nrp_cleserver/server/SimulationServer.py; \
          ...
          sed -i 's|NestClientControlAdapter()|NestClientControlAdapter(\"nid01383\", 5000)|' $NRPSRCSERV/server/ServerConfigurations.py;
          \
